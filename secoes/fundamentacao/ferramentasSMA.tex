...
\textit{Java Agent DEvelopment} \abreviatura{JADE}{Java Agent Development}(JADE)\cite{jade} é um \textit{framework} para desenvolvimento de aplicações de agentes compatível com as especificações da FIPA para interoperabilidade entre sistemas multiagentes, desenvolvido inicialmente pela Telecom Italia (que ainda é detentora de direitos autorais sobre a ferramenta), é distribuída em formato de código aberto sob as condições da LGPLv2 \cite{lgpl2}. O objetivo da ferramenta JADE é providenciar ao desenvolvedor uma maneira simples de desenvolver seu programa, facilitando a implementação dos padrões da FIPA. As características do JADE, importantes para o problema apresentado neste trabalho, são:
